...
def get_faces(detector, embedder, vs, d_conf):
    out_faces = []
    frame = vs.read()
...
def acquire_frame(detector, embedder, vs, recognizer, le, d_conf, r_conf):
    data = []
    # data = (face, vector, coords, name, proba), ...
    frame, out_faces = get_faces(detector, embedder, vs, d_conf)
